...
```json
{
    ...
    "dashboard": {
        ...
        "battle_plan": {
            ...
            "stock_lending": {
                "suitability": "推荐/可选/不建议",
                "suitability_score": 0-100,
                "reason": "是否适合参与股票借贷的核心原因",
                "expected_benefit": "可能收益：高/中/低；若无真实借券利率数据，明确说明无法估算具体收益",
                "main_risks": [
                  "借出股票可能影响投票权",
                  "借出股票可能被用于做空，价格波动风险仍由持有人承担",
                  "平台/借券安排可能不受 SIPC 同等保护，需以券商条款为准",
                  "若股票波动过大，借券收益可能远小于价格回撤风险"
                ],
                "holder_action": "已有持仓者是否建议开启 lending，以及是否应先减仓/止盈/观望",
                "final_suggestion": "开启/不开启/只对部分仓位开启"
            },
            "action_checklist": [
                "✅/⚠️/❌ 检查项1：多头排列",
                ...
                "✅/⚠️/❌ 检查项3：量能配合",
                "✅/⚠️/❌ 检查项4：无重大利空",
                "✅/⚠️/❌ 检查项5：Stock Lending 收益是否值得承担额外风险",
                "✅/⚠️/❌ 检查项6：PE估值合理"
            ]
        }
    },
...
## Stock Lending 评估规则
如果是美股或美股基金，请额外评估 Stock Lending 适合度：
1. 只评估是否适合参与股票借贷，不要假设已开启 lending。
2. 如果没有真实 borrow rate / lending rate 数据，必须明确写“未获得实时借券利率，无法估算具体收益”。
3. 重点比较：长期持有价值、短期波动风险、做空压力、流动性、估值/溢价风险、是否适合作为长期仓位。
4. 对高波动、题材炒作、NAV 大幅溢价、流动性不足、基本面不透明的标的，应倾向“不建议”或“只对小仓位可选”。
5. 对大型高流动性、长期持有意愿强、波动相对可控的标的，可给出“可选”，但仍需提示收益可能很低。
6. Stock Lending 建议不能覆盖原有买卖建议：如果股票本身应减仓/止盈，不得因为可借出赚收益而建议继续重仓持有。
    ...
    "dashboard": {
        ...
        "battle_plan": {
            ...
            "action_checklist": [
                ...
                "✅/⚠️/❌ 检查项7：Stock Lending 收益是否值得承担额外风险"
            ]
        }
    },
